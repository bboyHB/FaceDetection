...
if not os.path.exists(data_dir):
    ...
    all_face_img = os.listdir(face_dir)
    train_path, val_path = train_test_split(all_face_img, test_size=0.3)
    print("train_n:", len(train_path), 'val_n:', len(val_path))
    for x in train_path:
        src = os.path.join(face_dir, x)
        dst = os.path.join(data_dir, 'train', face_expressions[int(all_label[x]) - 1], x)
        shutil.copy(src, dst)
    for y in val_path:
        src = os.path.join(face_dir, y)
        dst = os.path.join(data_dir, 'val', face_expressions[int(all_label[y]) - 1], y)
        shutil.copy(src, dst)
